...
> **注意**：D1315 是 Delta（三角洲）打印机，Cura 配置与笛卡尔打印机有所不同，务必设置坐标原点在打印床中心。
...
| 参数 | 建议值 | 说明 |
|---|---|---|
| 打印速度 | 40–60 mm/s | Delta 机械特性，适当降速可提升精度 |
| 打印温度（PLA） | 190–205°C | — |
| 回抽距离 | 4–6 mm | Bowden 挤出机，需适当加大 |
| 回抽速度 | 40–50 mm/s | — |
| 风扇速度 | 100%（PLA） | 小机型散热需求高 |
...
M82                          ; 挤出机绝对模式
M107                         ; 关闭风扇
M109 S{material_print_temperature_layer_0}  ; 加热到打印温度并等待
...
G1 Z10 F3000                 ; 移到安全高度
G92 E0                       ; 重置挤出量
G1 X-55 Y0 Z0.3 F3000        ; 移到床边起点（Delta 原点在中心）
G1 X55  Y0 E8   F800         ; 打印引线（跨越左右直径）
G92 E0
G1 Z5 F3000                  ; 抬升 Z 轴
...
M400                         ; 等待当前动作完成
G92 E0
G1 E-3 F1800                 ; 快速回抽 3 mm
G91                          ; 相对坐标
G1 Z15 F3000                 ; 抬起 Z 轴 15 mm
G90                          ; 切回绝对坐标
G1 X0 Y0 F9000               ; 移回中心（Delta 归中）
M107                         ; 关闭冷却风扇
...
; ══ 等待降温后深度回抓（防漏料） ═════════════════
M109 R150                    ; 等待降至 150°C
G1 E-10 F300                 ; 深度回抽 10 mm
